...
(() => {
    ...
    // Reason: Rebuilds the `create` method for the LMBS Equip scene to properly
    // instantiate and display a help window, which was previously missing.
    Scene_LMBSEquip.prototype.create = function() {
        ...
        const wy = (Graphics.boxHeight / 2) - 180;
    ...
    // Reason: Provides compatibility with YEP_SkillCore for drawing skill costs.
    // It adjusts icon and text positions within the LMBS-specific windows to
    // prevent visual glitches and ensure proper alignment.
    if (Imported?.YEP_SkillCore) {
        ...
        const isLMBSScene = win =>
            (win instanceof LMBSwindowSkillEquip) || (win instanceof LMBSwindowSkillList);

        const makeCostDrawer = (type, iconId, color, format, fontSize) => {
            return function(skill, wx, wy, dw) {
                if (this._actor[`skill${type}Cost`](skill) <= 0) return dw;
                const iconWidth = Window_Base._iconWidth;
                const offsetIcon = isLMBSScene(this) ? customOffsets.iconX : 0;
                if (iconId > 0) {
                    const iw = wx + dw - iconWidth - offsetIcon;
                    this.drawIcon(iconId, iw, wy + customOffsets.iconY);
                    dw -= iconWidth + offsetIcon;
                }
                this.changeTextColor(this.textColor(color));
                const text = format.format(Yanfly.Util.toGroup(this._actor[`skill${type}Cost`](skill)), TextManager[`${type.toLowerCase()}A`]);
                this.contents.fontSize = isLMBSScene(this) ? fontSize : Yanfly.Param[`SCC${type}FontSize`];
                const newWX = wx + (isLMBSScene(this) ? customOffsets.textX : 0);
                const newDW = dw + (isLMBSScene(this) ? customOffsets.textExtraWidth : 0);
                this.drawText(text, newWX, wy + customOffsets.textY, newDW, 'right');
                const returnWidth = newDW - this.textWidth(text) - Yanfly.Param.SCCCostPadding;
                this.resetFontSettings();
                return returnWidth;
            };
        };

        Window_SkillList.prototype.drawTpCost = makeCostDrawer(
            'Tp', Yanfly.Icon.Tp, Yanfly.Param.SCCTpTextColor, Yanfly.Param.SCCTpFormat, customOffsets.tpFont
        );
        Window_SkillList.prototype.drawMpCost = makeCostDrawer(
            'Mp', Yanfly.Icon.Mp, Yanfly.Param.SCCMpTextColor, Yanfly.Param.SCCMpFormat, customOffsets.mpFont
        );
        Window_SkillList.prototype.drawHpCost = makeCostDrawer(
            'Hp', Yanfly.Icon.Hp, Yanfly.Param.SCCHpTextColor, Yanfly.Param.SCCHpFormat, customOffsets.hpFont
        );
    }
